...
describe('OperationsServiceImpl', () => {
  ...
  beforeEach(() => {
    ...
    // For paginator tests
    jest.spyOn(operationsService, 'getOperationPaginator').mockImplementation((orgId, ledgerId, accountId) => {
      if (!orgId) throw new ValidationError('Organization ID is required');
      if (!ledgerId) throw new ValidationError('Ledger ID is required');
      if (!accountId) throw new ValidationError('Account ID is required');
  ...
  describe('listOperations', () => {
    it('should list operations successfully', async () => {
      ...
      expect(operationApiClient.listOperations).toHaveBeenCalledWith(
        orgId, ledgerId, accountId, undefined
      );
    ...
    it('should apply list options when provided', async () => {
      ...
      // Verify
      expect(operationApiClient.listOperations).toHaveBeenCalledWith(
        orgId, ledgerId, accountId, listOptions
      );
    });
    
    it('should throw an error if orgId is missing', async () => {
      // Execute & Verify
      await expect(operationsService.listOperations('', ledgerId, accountId))
        .rejects.toThrow('Organization ID is required');
    });
    
    it('should throw an error if ledgerId is missing', async () => {
      // Execute & Verify
      await expect(operationsService.listOperations(orgId, '', accountId))
        .rejects.toThrow('Ledger ID is required');
    });
    
    it('should throw an error if accountId is missing', async () => {
      // Execute & Verify
      await expect(operationsService.listOperations(orgId, ledgerId, ''))
    ...
    it('should handle API errors', async () => {
      ...
      // Execute & Verify
      await expect(operationsService.listOperations(orgId, ledgerId, accountId))
  ...
  describe('getOperation', () => {
    it('should get an operation by ID successfully', async () => {
      ...
      expect(operationApiClient.getOperation).toHaveBeenCalledWith(
        orgId, ledgerId, accountId, operationId, undefined
      );
    ...
    it('should include transaction ID when provided', async () => {
      ...
      // Verify
      expect(operationApiClient.getOperation).toHaveBeenCalledWith(
        orgId, ledgerId, accountId, operationId, transactionId
      );
    });
    
    it('should throw an error if orgId is missing', async () => {
      // Execute & Verify
      await expect(operationsService.getOperation('', ledgerId, accountId, operationId))
        .rejects.toThrow('Organization ID is required');
    });
    
    it('should throw an error if ledgerId is missing', async () => {
      // Execute & Verify
      await expect(operationsService.getOperation(orgId, '', accountId, operationId))
        .rejects.toThrow('Ledger ID is required');
    });
    
    it('should throw an error if accountId is missing', async () => {
      // Execute & Verify
      await expect(operationsService.getOperation(orgId, ledgerId, '', operationId))
        .rejects.toThrow('Account ID is required');
    });
    
    it('should throw an error if operationId is missing', async () => {
      // Execute & Verify
      await expect(operationsService.getOperation(orgId, ledgerId, accountId, ''))
    ...
    it('should handle API errors', async () => {
      ...
      // Execute & Verify
      await expect(operationsService.getOperation(orgId, ledgerId, accountId, operationId))
        .rejects.toThrow(errorMessage);
    });
  });
  ...
  describe('updateOperation', () => {
    it('should update an operation successfully', async () => {
      ...
      expect(operationApiClient.updateOperation).toHaveBeenCalledWith(
        orgId, ledgerId, accountId, operationId, updateInput
      );
    ...
    it('should throw an error if orgId is missing', async () => {
      // Execute & Verify
      await expect(operationsService.updateOperation('', ledgerId, accountId, operationId, {}))
        .rejects.toThrow('Organization ID is required');
    });
    
    it('should throw an error if ledgerId is missing', async () => {
      // Execute & Verify
      await expect(operationsService.updateOperation(orgId, '', accountId, operationId, {}))
        .rejects.toThrow('Ledger ID is required');
    });
    
    it('should throw an error if accountId is missing', async () => {
      // Execute & Verify
      await expect(operationsService.updateOperation(orgId, ledgerId, '', operationId, {}))
        .rejects.toThrow('Account ID is required');
    });
    
    it('should throw an error if operationId is missing', async () => {
      // Execute & Verify
      await expect(operationsService.updateOperation(orgId, ledgerId, accountId, '', {}))
        .rejects.toThrow('Operation ID is required');
    });
    ...
    it('should handle API errors', async () => {
      ...
      // Execute & Verify
      await expect(operationsService.updateOperation(orgId, ledgerId, accountId, operationId, {}))
        .rejects.toThrow(errorMessage);
    });
  });
  ...
  describe('getAllOperations', () => {
    ...
    it('should throw an error if orgId is missing', async () => {
      ...
      // Execute & Verify
      await expect(operationsService.getAllOperations('', ledgerId, accountId))
        .rejects.toThrow('Organization ID is required');
    });
    
    it('should throw an error if ledgerId is missing', async () => {
      // Execute & Verify
      await expect(operationsService.getAllOperations(orgId, '', accountId))
        .rejects.toThrow('Ledger ID is required');
    });
    
    it('should throw an error if accountId is missing', async () => {
      // Execute & Verify
      await expect(operationsService.getAllOperations(orgId, ledgerId, ''))
        .rejects.toThrow('Account ID is required');
    });
  });
});