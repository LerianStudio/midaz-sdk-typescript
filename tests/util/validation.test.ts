...
describe('Validation Utilities', () => {
  ...
  describe('validatePattern', () => {
    it('should return valid result for matching pattern', () => {
      const result = validatePattern('ABC123', /^[A-Z0-9]+$/, 'testField', 'uppercase letters and numbers');
  ...
  describe('validateAssetCode', () => {
    ...
    it('should return invalid result for empty asset codes', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.message).toBe('assetCode must match an asset code format (3-8 uppercase letters or numbers)');
    });
  ...
  describe('validateMetadata', () => {
    ...
    it('should return invalid result for metadata with number value outside allowed range', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.message).toContain('metadata numeric value for key \'amount\' is outside of safe range');
      expect(result.fieldErrors).toBeDefined();
    });
