...
describe('Validation Edge Cases', () => {
  ...
  describe('String Validation Edge Cases', () => {
    ...
    it('should handle very long strings correctly', () => {
      ...
      const result2 = validatePattern(longString, maxLength9999Pattern, 'longString', 'string with max 9999 chars');
      expect(result2.valid).toBe(false);
      
      const result3 = validatePattern(longString, maxLength10000Pattern, 'longString', 'string with max 10000 chars');
      expect(result3.valid).toBe(true);
    });
    ...
    it('should handle strings with special characters correctly', () => {
      ...
      const result2 = validatePattern(specialChars, alphanumericPattern, 'specialChars', 'alphanumeric');
      expect(result2.valid).toBe(false);
    ...
    it('should handle strings with Unicode characters correctly', () => {
      ...
      const result1 = validatePattern(unicodeString, anyStringPattern, 'unicodeString', 'any string');
      expect(result1.valid).toBe(true);
      ...
      const result2 = validatePattern(unicodeString, minLength6Pattern, 'unicodeString', 'at least 6 chars');
      // We're expecting true here because the regex sees each unicode character as 1 character
      expect(result2.valid).toBe(true);
      
      const result3 = validatePattern(unicodeString, minLength5Pattern, 'unicodeString', 'at least 5 chars');
      expect(result3.valid).toBe(true);
    });
    ...
    it('should handle strings with line breaks correctly', () => {
      ...
      const result1 = validatePattern(multilineString, anyStringPattern, 'multilineString', 'any string');
      expect(result1.valid).toBe(true);
      
      const result2 = validatePattern(multilineString, singleLinePattern, 'multilineString', 'single line');
      expect(result2.valid).toBe(false);
      
      const result3 = validatePattern(multilineString, multilinePattern, 'multilineString', 'any string including newlines');
      expect(result3.valid).toBe(true);
    });
  });
