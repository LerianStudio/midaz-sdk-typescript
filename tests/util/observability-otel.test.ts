...
describe('OpenTelemetryProvider', () => {
  ...
  describe('startSpan', () => {
    ...
    it('should create a span when tracing is enabled', () => {
      ...
      const startSpanCall = calls.find(call => call[0] && call[0].includes('[OpenTelemetry] Starting span: test-span'));
    ...
    it('should set attributes on a span', () => {
      ...
      expect(console.debug).toHaveBeenCalled();
      const calls = (console.debug as jest.Mock).mock.calls;
      const setAttributeCall = calls.find(call => call[0] && call[0].includes('[OpenTelemetry] Span attribute: key=value'));
    ...
    it('should record exceptions on a span', () => {
      ...
      expect(console.debug).toHaveBeenCalled();
      const calls = (console.debug as jest.Mock).mock.calls;
      const recordExceptionCall = calls.find(call => call[0] && call[0].includes('[OpenTelemetry] Span exception: test error'));
    ...
    it('should set status on a span', () => {
      ...
      expect(console.debug).toHaveBeenCalled();
      const calls = (console.debug as jest.Mock).mock.calls;
      const setStatusCall = calls.find(call => call[0] && call[0].includes('[OpenTelemetry] Span status: error - something went wrong'));
    ...
    it('should end a span', () => {
      ...
      expect(console.debug).toHaveBeenCalled();
      const calls = (console.debug as jest.Mock).mock.calls;
      const endSpanCall = calls.find(call => call[0] && call[0].includes('[OpenTelemetry] Ending span: test-span'));
  ...
  describe('recordMetric', () => {
    ...
    it('should record a counter metric when metrics are enabled', () => {
      ...
      expect(console.debug).toHaveBeenCalled();
      const calls = (console.debug as jest.Mock).mock.calls;
      const counterCall = calls.find(call => call[0] && call[0].includes('Counter test-counter += 5'));
    ...
    it('should record a histogram metric when metrics are enabled', () => {
      ...
      expect(console.debug).toHaveBeenCalled();
      const calls = (console.debug as jest.Mock).mock.calls;
      const histogramCall = calls.find(call => call[0] && call[0].includes('Histogram test-histogram = -5'));
  ...
  describe('log', () => {
    ...
    it('should log a debug message when logging is enabled', () => {
      ...
      expect(console.debug).toHaveBeenCalled();
      const calls = (console.debug as jest.Mock).mock.calls;
      const debugCall = calls.find(call => call[0] && call[0].includes('[test-service] test debug message'));
    ...
    it('should log an info message when logging is enabled', () => {
      ...
      const infoCall = calls.find(call => call[0] && call[0].includes('[test-service] test info message'));
    ...
    it('should log a warning message when logging is enabled', () => {
      ...
      const warnCall = calls.find(call => call[0] && call[0].includes('[test-service] test warning message'));
    ...
    it('should log an error message when logging is enabled', () => {
      ...
      const errorCall = calls.find(call => call[0] && call[0].includes('[test-service] test error message'));
  ...
  describe('shutdown', () => {
    it('should log a debug message when shutting down', async () => {
      ...
      expect(console.debug).toHaveBeenCalled();
      const calls = (console.debug as jest.Mock).mock.calls;
      const shutdownCall = calls.find(call => call[0] && call[0].includes('[OpenTelemetry] Shutting down'));
  ...
  describe('tracer implementation', () => {
    ...
    it('should create a histogram through meter', () => {
      ...
      expect(console.debug).toHaveBeenCalled();
      const calls = (console.debug as jest.Mock).mock.calls;
      const histogramCall = calls.find(call => call[0] && call[0].includes('Histogram test-histogram = 42'));
      expect(histogramCall).toBeDefined();
    });
    ...
    it('should log through logger', () => {
      ...
      expect(console.debug).toHaveBeenCalled();
      const calls = (console.debug as jest.Mock).mock.calls;
      const debugCall = calls.find(call => call[0] && call[0].includes('[test-service] debug message'));
      expect(debugCall).toBeDefined();
      ...
      const infoCall = callsInfo.find(call => call[0] && call[0].includes('[test-service] info message'));
      expect(infoCall).toBeDefined();
      ...
      const warnCall = callsWarn.find(call => call[0] && call[0].includes('[test-service] warn message'));
      expect(warnCall).toBeDefined();
      ...
      const errorCall = callsError.find(call => call[0] && call[0].includes('[test-service] error message'));
      expect(errorCall).toBeDefined();
    });
  });
});
