...
export async function createInitialDeposit(
  ...
  if (mergedOptions.execute) {
    if (!mergedOptions.client || !mergedOptions.organizationId || !mergedOptions.ledgerId) {
      throw new Error(
        'Client, organization ID, and ledger ID must be provided when execute is true'
      );
    }
...
export async function createUserTransfer(
  ...
  const mergedOptions: MergedOptions = {
    ...
    scale: options.scale ?? 0,
    description:
      options.description ?? `Transfer from ${sourceAccountId} to ${destinationAccountId}`,
  };
...
export async function createRoundupSavings(
  ...
  const mergedOptions: MergedOptions = {
    ...
    scale,
    description:
      options.description ?? `Roundup savings from ${spendingAccountId} to ${savingsAccountId}`,
  };
...
export async function createRecurringPayment(
  ...
  const mergedOptions: MergedOptions = {
    ...
    description:
      options.description ?? `Recurring payment from ${sourceAccountId} to ${destinationAccountId}`,
  };
...
export function createCreditDebitPair(
  ...
  const creditDescription =
    description || `Credit to ${destinationAccountId} from ${sourceAccountId}`;
  const debitDescription =
    description || `Debit from ${destinationAccountId} to ${sourceAccountId}`;
