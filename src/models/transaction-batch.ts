...
export async function createTransactionBatch(
  ...
  const mergedOptions: Required<TransactionBatchOptions> = {
    ...
    onTransactionSuccess: options.onTransactionSuccess ?? (() => { /* empty success handler */ }),
    onTransactionError: options.onTransactionError ?? (() => { /* empty error handler */ }),
    batchMetadata: {
      batchId,
    ...
    async (transaction, index) => {
      try {
        ...
        if (txResult.status === 'success') {
          ...
          mergedOptions.onTransactionSuccess(transaction, index, txResult.result || { id: 'duplicate' });
        } else {
          result.failureCount++;
...
export async function createDepositBatch(
  ...
  // Process as a batch
  return createTransactionBatch(client, organizationId, ledgerId, transactions as CreateTransactionInput[], options);
}
