...
const updatedOrg = await httpClient.put('onboarding/organizations/org_12345', {
  status: 'inactive',
});
...
try {
  const document = await uploadDocument(httpClient, 'org_12345', '/path/to/statement.pdf');
...
try {
  await downloadReport(httpClient, 'report_12345', './reports/quarterly-summary.pdf');
...
async function processBankTransaction(client, transaction) {
  ...
  const result = await withEnhancedRecovery(
    async () => {
      ...
      const response = await client.post('transaction/transfers', transaction);

      // Verify the transaction was processed
      const status = await client.get(`transaction/transfers/${response.id}/status`);
