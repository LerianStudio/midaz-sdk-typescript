...
async function unorderedWorkerPoolExample(): Promise<void> {
  ...
  console.log('Processing items with varying times:', items.map(item => item.id));
  ...
  const results: { id: number, result: string, originalIndex: number }[] = await workerPool(
    items,
    async (item: { id: number, processingTime: number }, index: number): Promise<{ id: number, result: string, originalIndex: number }> => {
