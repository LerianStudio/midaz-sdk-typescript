...
function systemAccountExample() {
  ...
  accounts.forEach((account) => {
    console.log(
      `Account: ${account.name} (${account.id}) => System Account: ${isSystemAccount(account)}`
    );
  });
}
...
function groupAccountsByAssetExample() {
  ...
  Object.entries(accountsByAsset).forEach(([assetCode, accounts]) => {
    ...
    accounts.forEach((account) => {
      console.log(
        `- ${account.name} (${account.id}): ${account.balance} ${account.assetCode} (Ledger: ${account.ledgerId})`
      );
    });
  });
...
function accountDashboardExample() {
  ...
  Object.entries(userAccountsByAsset).forEach(([assetCode, accounts]) => {
    ...
    accounts.forEach((account) => {
      console.log(
        `- ${account.name}: ${account.balance} ${account.assetCode} (Type: ${account.type})`
      );
    });
  });
