...
function basicValidationExample() {
  ...
  console.log(`Validating pattern for "${zipCode}" (US zip code):`, 
    validatePattern(zipCode, /^\d{5}(-\d{4})?$/, 'ZipCode', 'a valid US zip code'));
  console.log(`Validating pattern for "${invalidZipCode}" (US zip code):`, 
  ...
  console.log(`Validating number ${validAge}:`, 
    validateNumber(validAge, 'Age', { min: 0, max: 120 }));
  console.log(`Validating number ${invalidAge}:`, 
  ...
  console.log(`Validating country code "${validCountry}":`, validateCountryCode(validCountry, 'Country'));
  console.log(`Validating country code "${invalidCountry}":`, validateCountryCode(invalidCountry, 'Country'));
}
...
function objectValidationExample() {
  ...
  try {
    ...
    if (!idResult.valid || !nameResult.valid || !ageResult.valid || !zipResult.valid) {
      ...
      if (!nameResult.valid && nameResult.fieldErrors) Object.assign(fieldErrors, nameResult.fieldErrors);
      if (!ageResult.valid && ageResult.fieldErrors) Object.assign(fieldErrors, ageResult.fieldErrors);
      if (!zipResult.valid && zipResult.fieldErrors) Object.assign(fieldErrors, zipResult.fieldErrors);
