...
function versionSpecificTransformersExample() {
  ...
  interface ApiV2Product {
    ...
    name: string;
    description: string;
    pricing: {  // Nested pricing object in v2
      amount: number;
      currency: string;
    };
    category: string;
    tags: string[];  // Array in v2
    inventory: {  // Nested inventory object in v2
      available: boolean;
      quantity?: number;
    };
    metadata: {  // Metadata added in v2
      created_at: string;
      updated_at?: string;
    };
  }
...
function apiResponseTransformExample() {
  ...
  const ordersArrayResponse: ApiV1Order[] = [
    singleOrderResponse,
    {
      ...
      customer_id: 'cust-123',
      line_items: [
        { product_id: 'prod-3', quantity: 1, unit_price: 19.99 }
      ],
      total_amount: 19.99,
      status: 'pending',
